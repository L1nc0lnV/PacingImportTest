CREATE PROCEDURE [dbo].[usp_Pacing_AutoTrader]
...
IF @QueryType = 'total'
    ...
    HAVING
		...
		SELECT [QRY - AutoTrader - Months Complete 1].[BudgetLookup], Sum([QRY - AutoTrader - Months Complete 1].[CountOfAssociate ID]) AS [SumOfCountOfAssociate ID]
		into #MonthComplete2
		FROM #MonthComplete1 [QRY - AutoTrader - Months Complete 1]
		GROUP BY [QRY - AutoTrader - Months Complete 1].[BudgetLookup];
        ...
        [QRY - AutoTrader - Months Complete 2].[SumOfCountOfAssociate ID] AS [Completed Months],
        [Agent Budgets].PaymentDeactivationDate,
        [Agent Budgets].[TargetDemo],
        Getdate() AS CreatedOn,
        'MediaManagerETL' AS CreatedBy,
        'AutoTrader' AS ProcessSource
    FROM
    ...
    INNER JOIN
        ...
        INNER JOIN
            (
                [dbo].[MedManAutoTargeting] [Auto Trader] 
            INNER JOIN
                #MonthComplete2 [QRY - AutoTrader - Months Complete 2] 
                    ON [Auto Trader].[BudgetLookup] = [QRY - AutoTrader - Months Complete 2].[BudgetLookup]
                ) 
            ...
            GROUP BY
                ...
                [QRY - Master Agent Profile].NumberOfTargetedZips,
                [Agent Budgets].[SBMSBudget],
                [QRY - AutoTrader - Months Complete 2].[SumOfCountOfAssociate ID],
                [Agent Budgets].PaymentDeactivationDate,
                [Agent Budgets].[TargetDemo];
