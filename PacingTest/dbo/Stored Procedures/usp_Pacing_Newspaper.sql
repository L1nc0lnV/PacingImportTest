CREATE PROCEDURE [dbo].[usp_Pacing_Newspaper]
...
IF @QueryType = 'total'
	...
	SELECT
        [Newspaper].[BudgetLookup],
        [Newspaper].[AssociateID],
        [Newspaper].Year,
        [Newspaper].MonthNum,
        [Newspaper].[DataType],
        Count([Newspaper].[AssociateID]) AS [CountOfAssociate ID],
        MonthAssign.FirstYear_MonthNumber,
        MonthAssign.SecondYear_MonthNumber
	into #MonthComplete1
    FROM
        @MonthAssign MonthAssign,
        [dbo].[MedManNewspaper] [Newspaper] 
    GROUP BY
        [Newspaper].[BudgetLookup],
        [Newspaper].[AssociateID],
        [Newspaper].Year,
    ...
    HAVING
		...
		SELECT [QRY - Newspaper - Months Complete 1].[BudgetLookup], Sum([QRY - Newspaper - Months Complete 1].[CountOfAssociate ID]) AS [SumOfCountOfAssociate ID]
		into #MonthComplete2
		FROM #MonthComplete1 [QRY - Newspaper - Months Complete 1]
		GROUP BY [QRY - Newspaper - Months Complete 1].[BudgetLookup];


		SELECT
        [Newspaper].[ReportingTactic],
        ...
        Sum([Newspaper].[PlannedImpressions]) AS [Total Planned Impressions],
        Count([Newspaper].[PlannedImpressions]) AS [Planned Months],
        Avg([Newspaper].[PlannedImpressions]) AS [Impressions Per Month],
        [Agent Budgets].[SBMSBudget],
        Sum([Newspaper].DeliveredImpressionsToDate) AS [Impressions to Date],
        Sum([Newspaper].Clicks) AS [Clicks to Date],
        Sum([Newspaper].Spend) AS [Spend to Date],
        [QRY - Newspaper - Months Complete 2].[SumOfCountOfAssociate ID] AS [Completed Months],
        [Agent Budgets].PaymentDeactivationDate,
        [Agent Budgets].[TargetDemo],
        Getdate() AS CreatedOn,
        'MediaManagerETL' AS CreatedBy,
        'Newspaper' AS ProcessSource
    FROM
    ...
    INNER JOIN
        ...
        INNER JOIN
            (
                [dbo].[MedManNewspaper] [Newspaper] 
            INNER JOIN
                #MonthComplete2 [QRY - Newspaper - Months Complete 2] 
                    ON [Newspaper].[BudgetLookup] = [QRY - Newspaper - Months Complete 2].[BudgetLookup]
                ) 
                    ON [QRY - Master Agent Profile].[AssociateID] = [Newspaper].[AssociateID]
                ) 
                    ON [Agent Budgets].AssociateIDTactic = [Newspaper].[BudgetLookup] 
            GROUP BY
                [Newspaper].[ReportingTactic],
                [QRY - Master Agent Profile].[MarketArea],
                [QRY - Master Agent Profile].MediaState,
                ...
                [QRY - Master Agent Profile].NumberOfTargetedZips,
                [Agent Budgets].[SBMSBudget],
                [QRY - Newspaper - Months Complete 2].[SumOfCountOfAssociate ID],
                [Agent Budgets].PaymentDeactivationDate,
                [Agent Budgets].[TargetDemo];
